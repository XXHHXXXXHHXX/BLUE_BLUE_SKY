...
const VRNode: React.FC<NodeProps> = (props) => {
  ...
  const renderField = (fieldKey: string) => {
    switch (fieldKey) {
      ...
      case 'vrOutputVoltageControl':
        return (
          <InteractiveZone key={fieldKey}>
            <span style={{ fontSize: 11, color: '#666' }}>{getFieldLabel('vrOutputVoltageControl')}: {outputVoltage !== null && !Number.isNaN(outputVoltage) ? `${outputVoltage.toFixed(2)}V` : 'NA'}</span>
            <input
              type="range"
              min={range.min}
              max={range.max}
