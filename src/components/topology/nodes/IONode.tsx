...
const IONode: React.FC<NodeProps> = (props) => {
  ...
  const renderField = (fieldKey: string) => {
    switch (fieldKey) {
      ...
      case 'linkSpeed':
        return hasField('linkSpeed') ? <div key={fieldKey}>{getFieldLabel('linkSpeed')}: {linkSpeed ?? 'NA'}</div> : null;
      default: {
  ...
  // 处理 null 值
  const power = io?.power ?? null;
  const temperature = io?.temperature ?? null;
  const linkSpeed = io?.linkSpeed ?? null;
